...
describe('de-nest is working properly for', () => {
  describe('#array', () => {
    ...
    it('reduce array method should give array syntax top level with either as it nests deeper', () => {
      assert.equal(
        JSON.stringify(reduceArray(['arr', arr])),
        '{"arr[0]":7,"arr[2].value2":4,"arr[3]":1,"arr[4]":2,"arr[5]":3,"arr[6]":4}'
      );
    })
    ...
    it('reduce a array method should not give object syntax top level', () => {
      assert.notEqual(
        JSON.stringify(reduceArray(['obj', obj])),
        '{"obj.a":7,"obj.b":2,"obj.c.value":3}'
      );
    })
    ...
    it('reduce b array method should not give object syntax top level', () => {
      assert.equal(
        JSON.stringify(reduceArray(['obj', obj])),
        '"What did you do 😱 , this is not an array... do not worry I will fix it."');
    })
  })
  describe('#object', () => {
    ...
    it('flattenAnything, evaluate an object as an object', () => {
      assert.equal(
        JSON.stringify(flattenAnything({obj})),
        '{"keys":["obj.a","obj.b","obj.c.value"],"values":[7,2,3]}'
      );
    })

    it('handleNested, evaluate an object as an object', () => {
      assert.equal(
        JSON.stringify(handleNested({obj})),
        '{"obj.a":7,"obj.b":2,"obj.c.value":3}'
      );
    })
    ...
    it('reduce object method should give object syntax top level', () => {
      assert.equal(
        JSON.stringify(reduceObject(['obj', obj])),
        '{"obj.a":7,"obj.b":2,"obj.c.value":3}'
      );
    })
    ...
    it('reduce object method should not give array syntax top level', () => {
      assert.notEqual(
        JSON.stringify(reduceObject(['arr', arr])),
        '{"arr[0]":7,"arr[2].value2":4,"arr[3]":1,"arr[4]":2,"arr[5]":3,"arr[6]":4}'
      );
    })
  })
  describe('#merge', () => {
    ...
    it('two objects together', () => {
      assert.equal(
        JSON.stringify(ecma2015ArrMerge(mergeObj1, mergeObj2)),
        '{"c":{"value":3},"d":2,"a":7,"b":2}'
      );
    })
  })
  ...
  describe('#convert', () => {
    ...
    assert.equal(
      JSON.stringify(convertObjectToArrays(obj)),
      '{"keys":["obj.a","obj.b","obj.c.value"],"values":[7,2,3]}'
    )
  })
})
